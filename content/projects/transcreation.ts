...
export const transcreation: Project = {
  ...
  hero: {
    ...
    alt: {
      ...
      zh: '汉译英语料表：原片名、上映年份、影片类型、英文官方译名，以及 DeepSeek R1 和 Gemini 2.5 Pro 各自的输出，右侧附中英海报。',
    },
  },
  links: [],
  facts: [
  ...
  sections: [
    ...
    },
    {
      ...
      caption: {
        en: 'Where this started: a course poster for an English-Chinese contrastive translation class, sorting film titles into ten strategy categories. The paper exists because of what is wrong with it. Categories at this level record **what was done**, such as literal rendering, domestication or adding context, and say nothing about why one choice worked and its neighbour did not. Moving from a label to a mechanism is the whole distance between that poster and the three-tier model.',
        zh: '起点是这张海报：英汉对比翻译课上的小组展示，把片名分到十个策略类别里。后来的论文，恰恰是从这张海报的不足长出来的。这个层级的分类记下的是**做了什么**，比如直译、归化、增补语境，却完全没说清为什么这个选择成立、旁边那个不成立。从一个标签走到一套机制，就是这张海报和三层模型之间的全部距离。',
      },
    },
    {
      kind: 'prose',
